...
int main()
{
    ...
    while(game)
    {
        ...
        if(snakeX[0] == foodX && snakeY[0] == foodY) // checking snake and food is in the same position or not
        {
            length = length + 2;
            food = food + 1 ;
            delayTime = delayTime - 25;  // food consumed by snake then the delay time reduced and snake spread increase
        ...
        //adding score
        char ara[100];
        sprintf(ara,"Score: %d",(food-1)*10);
        setcolor(LIGHTBLUE);
        settextstyle(6,0,2);
        outtextxy(220,maxy,ara);
